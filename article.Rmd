...
kable(bal_df, "html", caption = "Balance Statistics For Unfunded Control Group") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
# Display the cleaned-up table with kable
kable(bal_df_funded, "html", caption = "Balance Statistics For Funded Control Group") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
# summary(model_gls_interactions_subclass)
...
<!-- In the updated analysis using a linear mixed model, we examined the effects of treatment, independence_timing, and their interaction on the dependent variable, researcher independence (RII) for the treatment group and unfunded control group. -->

<!-- - There was a significant main effect of `independence_timing` on RII, t(`r summary(model)$coefficients[3,3]`) = `r summary(model)$coefficients[3, "t value"]`, p = `r format(summary(model)$coefficients[3,5], scientific = TRUE)`, suggesting that there was a significant change in RII from before to after intervention in the control group. -->

<!-- - There was also a significant main effect of `treatment` on RII, t(`r summary(model)$coefficients[2,3]`) = `r summary(model)$coefficients[2, "t value"]`, p = `r format(summary(model)$coefficients[2,5], scientific = TRUE)`. This effect indicates a difference in RII between the control group and the treatment group before the intervention. -->

<!-- - Importantly, there was a significant interaction effect between `treatment` and `independence_timing` on RII, t(`r summary(model)$coefficients[4,3]`) = `r summary(model)$coefficients[4, "t value"]`, p = `r format(summary(model)$coefficients[4,5], scientific = TRUE)`. This interaction suggests that the effect of the intervention on independence differed based on the timing of the measurement. The effect size for all fixed effects can be approximated using the marginal \( R^2 \) which is `r r.squaredGLMM(model)[1]`, suggesting that about `r round(r.squaredGLMM(model)[1] * 100, 1)`% of the variance in RII can be explained by the main effects in our model. The effect size for the interaction specifically can be computed using \( \Delta R^2 \) due to the interaction, which is `r R2_interaction`, indicating that about `r round(R2_interaction * 100, 1)`% of variance in RII is explained by the interaction between treatment and independence_timing over and above the main effects. -->
